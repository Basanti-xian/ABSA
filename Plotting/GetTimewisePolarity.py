...
def PlotCatPolarityScatter(movingAvg_cat_polarity_grouped_merged_mat_list, timeline, cat_list, names):
    ...
    for name_ind, mat in enumerate(movingAvg_cat_polarity_grouped_merged_mat_list):
        ...
        p = plt.scatter(ind, mat, alpha=1, marker=marker, color=color)
        p = plt.plot(ind, mat, color=color, linewidth=1.5)
        plots.append(p)
    ...
    if GroundTruth_flag:
        title = 'GroundTruth Polarity Distribution'
    else:
        title = 'SVM Prediction Polarity Distribution'
    plt.title(title)
...
def main():
    ...
    time_dict_list = []
    for f_ind, senlist_df in enumerate(senlist_df_list):
        ...
        for ind, date in enumerate(senlist_df.Date):
            ...
            if GroundTruth_flag:
                ...
                ###for SVM prediction
                pred = [float(l) for l in open(Pred_fnames[f_ind]).xreadlines()]
                p = pred[ind]
